...
- 已上传的照片以**缩略图 + 文件名**的形式平铺在弹窗里（`ElImage`，可点击放大、左右切换预览），不再是只列文件名。图片来源是 `GET /api/appeal/photo?key=<记录ID>&filename=<生成名>`：文件名复用删除那套严格校验（小写化、白名单扩展名 `.jpg/.jpeg/.png/.webp`、必须匹配服务端生成格式 `<16位小写hex>_<6位小写hex><ext>`），再用 `safeExistingAppealPhotoPath` 做软链接与目录越界检查，最后才交给 `http.ServeFile`，因此路径穿越、伪造文件名、越界符号链接都会被拒（返回 400/404）。
- 照片仍存放在 `config/evidence_<记录ID>/` 下，导出 zip 的逻辑不变。
...
常规扣分记录的「导入 Excel」还兼容学校下发的 **《警务化管理日常检查表》**（老式 `.xls`，见 `handlers/inspection.go:parseInspectionRows`）。注意这是真正 BIFF8 的 `.xls`（OLE2 复合文档），**excelize 读不了**，所以：

- 上传接口按**文件头**判断（`D0 CF 11 E0` = OLE2），`.xls` 走 `github.com/shakinm/xlsReader` 读成 `[][]string`，`.xlsx` 仍走 excelize；两者随后共用 `importDeductionRows` 这一套版式识别。前端 `accept` 也放开了 `.xls`。
- 版式：第 1 行是标题 + 「检查日期：x年x月x日」，第 2 行表头为 `区队 | 内务组 | 扣分分值 | 警容风纪组 | 扣分分值 | 生活秩序 | 扣分分值 | 总扣分`，之后**每个区队占一段**（区队名只在该段首行，向下沿用）。
- 只取 `区队` 列等于当前配置区队（工作台「区队」）那一段，并按 `内务组` / `警容风纪组` / `生活秩序` 三列取出内容；各自右侧的「扣分分值」列作为分数，非数字（如「提醒」）记 0 分。
- 导入日期取标题里的「检查日期」并补成 `YYYY-MM-DD`，**时分秒取导入时的当前时刻**。
- 每行内容按 `splitInspectionItem` 拆成「姓名 + 扣分项目」，规则（时间关键词：`上午 早二 早操 早一 下午 中午 傍晚 晚上 晚点名前 晚点名`，取行内最靠前、同位取最长）：

  | 原内容 | 姓名 | 扣分项目 |
  |---|---|---|
  | `张三 上午 腰带摆放不规范` | 张三 | `上午 腰带摆放不规范` |
  | `李四早二阳台晾衣杆摆放有问题` | 李四 | `早二阳台晾衣杆摆放有问题` |
  | `王五 早一离寝未断电` | 王五 | `早一离寝未断电` |
  | `早二包干区脏` | 区队名称 | `早二包干区脏` |
  | `早二 10001消防箱上有杂物` | 区队名称 | `早二 10001消防箱上有杂物` |
  | `赵六 滑柜未完全贴进`（无时间关键词） | 赵六 | `滑柜未完全贴进` |
  | `10002 早二 厕所灯未关` | 10002 | `早二 厕所灯未关` |
  | `10003孙七上午地面脏` | 孙七 | `上午地面脏` |

  （上表姓名与编号均为占位示例；真实姓名只在导入时从本地 `students` 表匹配，不写入文档或日志。）

- 姓名只按原样记入 `student_name`，能不能认到学生由 `persistDeductionRows` 按姓名匹配 `students` 决定：**匹配不到就是「未认定」**（如上面的 `区队名称`、`10002`），可在扣分记录管理或本周扣分条目汇总里用「编辑认定」补上。
- 同一张表里"姓名+内容+分数"完全相同的条目只导一次，避免同一秒内生成同一个记录 ID 而互相顶掉；导入类型恒为**大队督察**（不加 `xd_` 前缀）。
- 依赖 `github.com/shakinm/xlsReader`（纯 Go），已随项目缓存；本机 `go build` 离线可用，换机器构建需要能访问 Go 模块代理。
