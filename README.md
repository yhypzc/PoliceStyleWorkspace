...
- **未认定的记录也会列出**（`deductionDetailRows`）：常规扣分没有任何认定学生时，整条记录的分数记成一行、姓名列用记录里保存的原始姓名字段；寝室整体差子项没有负责学生时按 `record.Score / 子项数` 记成一行。所以明细表分数列的合计就等于该周列出的全部扣分项之和，与 `include_weekly`、认定状态无关。（"无子项"的寝室整体差主记录仍不列出，它属于工作台单独统计的条目。）
