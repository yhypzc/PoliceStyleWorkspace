...
### 信网大队《警务化管理日常检查表》(.xls) 导入

常规扣分记录的「导入 Excel」还兼容学校下发的 **《信网大队警务化管理日常检查表》**（老式 `.xls`，见 `handlers/inspection.go:parseInspectionRows`）。注意这是真正 BIFF8 的 `.xls`（OLE2 复合文档），**excelize 读不了**，所以：
